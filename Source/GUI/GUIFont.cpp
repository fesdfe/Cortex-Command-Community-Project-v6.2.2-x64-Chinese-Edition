...
bool GUIFont::Load(GUIScreen* Screen, const std::string& Filename) {
	...
	memset(m_Characters, 0, sizeof(Character) * 256 * 256);
	...
	// gbk汉字编码
	for (int chr = 0xb0a0; chr < 0xf7ff; chr++) {
		if (chr % 256 < 0xa0)
			chr = (chr / 256) * 256 + 0xa0;
		m_Characters[chr].m_Offset = x;
...
void GUIFont::Draw(GUIBitmap* Bitmap, int X, int Y, const std::string& Text, unsigned long Shadow, int iconFlag) {
	unsigned int c;
	GUIRect Rect;
	...
	for (int i = 0; i < Text.length(); i++) {
		...
		// 当字符标志为真时，跳过双字符转换
		if (iconFlag == 0)
			if ((Text.at(i) < 0) && (Text.length() > i + 1)) {
				if (Text.at(i + 1) < 0) {
					c = (256 + Text.at(i)) * 256 + 256 + Text.at(i + 1);
					i++;
				} else
					c = 256 + Text.at(i); // 兼容单个特殊字符
			} else
				c = Text.at(i);
		else {
			c = Text.at(i) < 0 ? 256 + Text.at(i) : Text.at(i);
		}
		...
		if (c < 256)
			...
			offY = ((c / 0x100 - 0xb0) * 6 + (c % 0x100 - 0xa0) / 0x10 + 14) * m_FontHeight;
...
void GUIFont::DrawAligned(GUIBitmap* Bitmap, int X, int Y, const std::string& Text, int HAlign, int VAlign, int MaxWidth, unsigned long Shadow, int iconFlag) {
	...
	while (lineStartPos < Text.size()) {
		...
		// 自动换行，这个地方依据空格隔断，所以想让汉字支持自动换行必须也加空格
		...
		if ((yLine + m_FontHeight) >= 0) {
			switch (HAlign) {
				...
				case Left:
					Draw(Bitmap, X, yLine, TextLine, Shadow, iconFlag);
					break;

					// Center HAlignment: Where X is the center point of the text
				case Centre:
					Draw(Bitmap, X - lineWidth / 2, yLine, TextLine, Shadow, iconFlag);
					break;

					// Right HAlignment: Where X is the end point of the text
				case Right:
					Draw(Bitmap, X - lineWidth, yLine, TextLine, Shadow, iconFlag);
					break;
				default:
					break;
			}
...
int GUIFont::CalculateWidth(const std::string& Text, int iconFlag) {
	unsigned int c;
	int Width = 0;
	int WidestLine = 0;
...
int GUIFont::CalculateHeight(const std::string& Text, int MaxWidth, int iconFlag) {
	if (Text.empty()) {
		return 0;
	}
	unsigned int c;
