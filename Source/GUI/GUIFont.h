...
namespace RTE {
	...
	public:
		...
		void Draw(GUIBitmap* Bitmap, int X, int Y, const std::string& Text, unsigned long Shadow = 0, int iconFlag = 0);

		/// Draws text to a bitmap aligned.
		/// @param Bitmap Bitmap, Position, Text.
		void DrawAligned(GUIBitmap* Bitmap, int X, int Y, const std::string& Text, int HAlign, int VAlign = Top, int maxWidth = 0, unsigned long Shadow = 0, int iconFlag = 0);
		...
		/// @param Text Text.
		int CalculateWidth(const std::string& Text, int iconFlag = 0);
		...
		int CalculateHeight(const std::string& Text, int MaxWidth = 0, int iconFlag = 0);
	...
	private:
		...
		std::string m_Name;
		Character m_Characters[256 * 256];
