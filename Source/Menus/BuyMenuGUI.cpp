	...
	*/
	...
	// ITEMS LIST focus
		...
		if (pItem && pItem->m_pEntity) {
			description = ((pItem->m_pEntity->GetDescription().empty()) ? "-未找到描述-" : pItem->m_pEntity->GetDescription()) + "\n"; //-No Information Found-
			...
			if (itemAsCraft) {
				...
				if (craftMaxMass == 0) {
					description += "\n无仓储容量 !"; //NO CARGO SPACE!
				} else if (craftMaxMass > 0) {
					description += "\n最大承载质量 : " + RoundFloatToPrecision(craftMaxMass, craftMaxMass < 50.0F ? 1 : 0, 3) + " kg"; //Max Mass
				}
				if (craftMaxPassengers >= 0 && craftMaxMass != 0) {
					description += (craftMaxPassengers == 0) ? "\n无人员容量 !" : "\n最大人员容量 : " + std::to_string(craftMaxPassengers); //NO PASSENGER SPACE! : Max Passengers:
				}
			} else {
				...
				if (itemAsActor) {
					description += "\n质量 : " + (itemAsActor->GetMass() < 0.1F ? "<0.1 kg" : RoundFloatToPrecision(itemAsActor->GetMass(), itemAsActor->GetMass() < 50.0F ? 1 : 0, 3) + " kg"); //Mass:
					int passengerSlotsTaken = itemAsActor->GetPassengerSlots();
					if (passengerSlotsTaken > 1) {
						description += "\n人员容量占用 : " + std::to_string(passengerSlotsTaken); //Passenger Slots:
					}
				} else {
					...
					if (itemAsMO) {
						...
						if (itemAsMOSRotating) {
							if (itemAsMOSRotating->NumberValueExists("Grenade Count")) {
								description += "\n手雷数量 : " + RoundFloatToPrecision(itemAsMOSRotating->GetNumberValue("Grenade Count"), 0, 2); //Grenade Count:
							}
							if (itemAsMOSRotating->NumberValueExists("Replenish Delay") && itemAsMOSRotating->GetNumberValue("Replenish Delay") > 0) {
								description += "\n投放延迟 : " + RoundFloatToPrecision(itemAsMOSRotating->GetNumberValue("Replenish Delay") / 1000.0F, 3, 2) + " seconds"; //Replenish Delay:
							}
							if (itemAsMOSRotating->NumberValueExists("Belt Mass")) {
								extraMass = itemAsMOSRotating->GetNumberValue("Belt Mass");
							}
						}
						description += "\n质量 : " + (itemAsMO->GetMass() + extraMass < 0.1F ? "<0.1 kg" : RoundFloatToPrecision(itemAsMO->GetMass() + extraMass, itemAsMO->GetMass() + extraMass < 50.0F ? 1 : 0, 3) + " kg"); //Mass:
					}
				}
			}
	...
	// CART/ORDER LIST focus
		...
		if (pItem && pItem->m_pEntity) {
			...
			if (itemAsActor) {
				...
				const MovableObject* itemAsMO = dynamic_cast<const MovableObject*>(currentItem);
				if (itemAsMO) {
					description += "\n质量 : " + (itemAsMO->GetMass() < 0.1F ? "<0.1 kg" : RoundFloatToPrecision(itemAsMO->GetMass(), itemAsMO->GetMass() < 50.0F ? 1 : 0, 3) + " kg"); //Mass:
				}
			}
		}
		...
		if (!catalogList[moduleID].empty()) {
			...
			if (!tempList.empty()) {
				...
				if (moduleID != 0 && (pModule = g_PresetMan.GetDataModule(moduleID))) {
					...
					// 购买菜单的阵营的那个玩意的汉化
					std::string name = CN_getNameFromDescription(pModule->GetDescription(), pModule->GetFriendlyName());
...
bool BuyMenuGUI::DeployLoadout(int index) {
	...
	if (m_Loadouts[index].GetDeliveryCraft()) {
		...
		// Take into account whether these are native or not, and multiply the cost accordingly
		// 好像是统计信息的投送仓名字，汉化一下
		m_pCraftBox->SetText(CN_getNameFromDescription(m_pSelectedCraft->GetDescription(), m_pSelectedCraft->GetPresetName()));
		m_pCraftBox->SetRightText(m_pSelectedCraft->GetGoldValueString(m_NativeTechModule, m_ForeignCostMult));

		m_pCraftNameLabel->SetText(CN_getNameFromDescription(m_pSelectedCraft->GetDescription(), m_pSelectedCraft->GetPresetName()));
...
void BuyMenuGUI::UpdateTotalCostLabel(int whichTeam) {
	std::string display = "总消费 : " + RoundFloatToPrecision(GetTotalOrderCost(), 0, 2) + "/" + RoundFloatToPrecision(g_ActivityMan.GetActivity()->GetTeamFunds(whichTeam), 0); //Cost:
