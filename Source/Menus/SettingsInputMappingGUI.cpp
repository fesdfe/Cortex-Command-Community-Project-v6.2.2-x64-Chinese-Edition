...
void SettingsInputMappingGUI::SetEnabled(bool enable, int player) {
	...
	if (enable) {
		m_InputMappingSettingsLabel->SetText("玩 家  " + std::to_string(player + 1) + "  按 键 映 射"); //"P L A Y E R   "   "   I N P U T   M A P P I N G"
...
void SettingsInputMappingGUI::UpdateMappingButtonLabels() {
	...
	for (int i = 0; i < InputElements::INPUT_COUNT; ++i) {
		...
		m_InputMapButton[i]->SetText(!inputDescription.empty() ? "[" + inputDescription + "]" : "[未定义]" /*"[Undefined]"*/);
	}
