...
void SettingsInputGUI::UpdatePlayerSelectedDeviceLabel(int player) {
	...
	switch (g_UInputMan.GetControlScheme(player)->GetDevice()) {
		case InputDevice::DEVICE_KEYB_ONLY:
			deviceLabel = "½ö¼üÅÌ"; //Keyboard Only
			break;
		case InputDevice::DEVICE_MOUSE_KEYB:
			deviceLabel = "¼üÊó"; //Mouse + Keyboard
			break;
		case InputDevice::DEVICE_GAMEPAD_1:
			deviceLabel = "¿ØÖÆÆ÷ 1"; //Gamepad 1
			break;
		case InputDevice::DEVICE_GAMEPAD_2:
			deviceLabel = "¿ØÖÆÆ÷ 2"; // Gamepad 2
			break;
		case InputDevice::DEVICE_GAMEPAD_3:
			deviceLabel = "¿ØÖÆÆ÷ 3"; // Gamepad 3
			break;
		case InputDevice::DEVICE_GAMEPAD_4:
			deviceLabel = "¿ØÖÆÆ÷ 4"; // Gamepad 4
			break;
		default:
			break;
	}
...
void SettingsInputGUI::UpdatePlayerInputSensitivityControlValues(int player) {
	switch (g_UInputMan.GetControlScheme(player)->GetDevice()) {
		...
		case InputDevice::DEVICE_MOUSE_KEYB:
			...
			for (int otherPlayer = Players::PlayerOne; otherPlayer < Players::MaxPlayerCount; ++otherPlayer) {
				if (g_UInputMan.GetControlScheme(otherPlayer)->GetDevice() == InputDevice::DEVICE_MOUSE_KEYB) {
					...
					m_PlayerInputSettingsBoxes.at(otherPlayer).SensitivityLabel->SetText("Êó±êÁéÃô¶È : " + std::to_string(m_PlayerInputSettingsBoxes.at(otherPlayer).SensitivitySlider->GetValue())); //Mouse Sensitivity:
				}
			}
			break;
		case InputDevice::DEVICE_GAMEPAD_1:
		...
		case InputDevice::DEVICE_GAMEPAD_4:
			...
			m_PlayerInputSettingsBoxes.at(player).SensitivityLabel->SetText("Ò¡¸ËËÀÇø : " + std::to_string(m_PlayerInputSettingsBoxes.at(player).SensitivitySlider->GetValue())); //Stick Deadzone:
