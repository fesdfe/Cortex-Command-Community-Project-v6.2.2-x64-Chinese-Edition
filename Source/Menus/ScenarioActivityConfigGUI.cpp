...
void ScenarioActivityConfigGUI::PopulateTechComboBoxes() {
	for (int team = Activity::Teams::TeamOne; team < Activity::Teams::MaxTeamCount; ++team) {
		m_TeamTechComboBoxes[team]->GetListPanel()->AddItem("-全部-", "", nullptr, nullptr, -2); //-All-
		m_TeamTechComboBoxes[team]->GetListPanel()->AddItem("-随机-", "", nullptr, nullptr, -1);//-Random-
	...
	for (int moduleID = 0; moduleID < g_PresetMan.GetTotalModuleCount(); ++moduleID) {
		if (const DataModule* dataModule = g_PresetMan.GetDataModule(moduleID)) {
			if (dataModule->IsFaction()) {
				for (int team = Activity::Teams::TeamOne; team < Activity::Teams::MaxTeamCount; ++team) {
					m_TeamTechComboBoxes[team]->GetListPanel()->AddItem(CN_getNameFromDescription(dataModule->GetDescription(), dataModule->GetFriendlyName()), "", nullptr, nullptr, moduleID);
				}
			}
		}
	}
...
void ScenarioActivityConfigGUI::ResetActivityConfigBox() {
	...
	for (int team = Activity::Teams::TeamOne; team < Activity::Teams::MaxTeamCount; ++team) {
		...
		if (m_SelectedActivity->TeamActive(team)) {
			...
			m_TeamNameLabels.at(team)->SetText("不可用"); //Unavailable
		}
		if (teamIcon) {
...
bool ScenarioActivityConfigGUI::Update(int mouseX, int mouseY) {
	...
	if (humansInTeams > maxHumanPlayers) {
		errorMessage = "分配了太多玩家 ! 该行动的最大玩家数为 %d" + std::to_string(maxHumanPlayers); //"Too many players assigned! Max for this activity is %d"
	} else if (minTeamsRequired > teamsWithPlayers) {
		errorMessage = "分配至少 " + std::to_string(minTeamsRequired) + " 支队伍 !"; //"Assign players to at least " + std::to_string(minTeamsRequired) + " of the teams!"
	} else if (teamWithHumans == 0) {
		errorMessage = "分配至少一支队伍 !"; //"Assign human players to at least one team!"
	}
...
void ScenarioActivityConfigGUI::UpdateStartingGoldSliderAndLabel() {
	...
	if (m_StartingGoldSlider->GetValue() == m_StartingGoldSlider->GetMaximum()) {
		std::snprintf(goldString.data(), goldString.size(), " %c 无限", -58); //Infinite
	} else {
		int startGold = m_StartingGoldSlider->GetValue();
		std::snprintf(goldString.data(), goldString.size(), " %c %d 盎司", -58, startGold);
	}
