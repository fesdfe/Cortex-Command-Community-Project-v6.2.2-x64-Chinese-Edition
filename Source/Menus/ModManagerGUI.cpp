...
void ModManagerGUI::ToggleMod() {
	...
	if (index > -1) {
		...
		if (modRecord.Disabled) {
			m_ToggleModButton->SetText("启用Mod" /*"Enable Mod"*/);
			...
			m_ToggleModButton->SetText("禁用Mod" /*"Disable Mod"*/);
...
void ModManagerGUI::ToggleScript() {
	...
	if (index > -1) {
		...
		if (scriptRecord.Enabled) {
			m_ToggleScriptButton->SetText("禁用脚本"); //Disable Script
			...
			m_ToggleScriptButton->SetText("启用脚本"); //Enable Script
	...
	while (m_GUIControlManager->GetEvent(&guiEvent)) {
		if (guiEvent.GetType() == GUIEvent::Command) {
			...
			if (guiEvent.GetControl() == m_ModsListBox && (guiEvent.GetMsg() == GUIListBox::Select && m_ModsListBox->GetSelectedIndex() > -1)) {
				...
				m_ToggleModButton->SetText(modRecord.Disabled ? "启用模组" : "禁用模组"); //"Enable Mod" : "Disable Mod"
				...
				m_ToggleScriptButton->SetText(scriptRecord.Enabled ? "禁用脚本" : "启用脚本"); //"Disable Script" : "Enable Script"
			}
		}
	}
	return false;
