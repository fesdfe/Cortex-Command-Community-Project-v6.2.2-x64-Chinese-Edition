...
void MetagameGUI::SetEnabled(bool enable) {
	...
	if (!m_TechAndFlagListFetched) {
		...
		for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
			m_apPlayerTechSelect[player]->GetListPanel()->AddItem("-随机-", "", nullptr, nullptr, -1); //-Random-
		...
		for (int moduleID = 0; moduleID < g_PresetMan.GetTotalModuleCount(); ++moduleID) {
			if (const DataModule* dataModule = g_PresetMan.GetDataModule(moduleID)) {
				if (dataModule->IsFaction()) {
					for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
						m_apPlayerTechSelect[player]->GetListPanel()->AddItem(CN_getNameFromDescription(dataModule->GetDescription(), dataModule->GetFriendlyName()), "", nullptr, nullptr, moduleID);
					}
				}
			}
		}
...
bool MetagameGUI::StartNewGame() {
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		...
		if (m_apPlayerControlButton[player]->GetText() != "None") {
			...
			newPlayer.SetHuman(m_apPlayerControlButton[player]->GetText() == "玩家"); //Human
...
void MetagameGUI::Update() {
	...
	*/
	...
	if (m_MenuScreen == NEWDIALOG) {
		...
		if (m_ScreenChange) {
			...
			if (autoSaveIndex >= 0 && pAutoSave) {
				...
				std::snprintf(info, sizeof(info), "游戏规模 : %d 个站点\n总玩家数 : %d\n天数 : %d\n%s", pAutoSave->GetSiteCount(), pAutoSave->GetPlayerCount(), pAutoSave->GetRoundCount() + 1, difficultyString.c_str()); //"Game Size: %d sites\nTotal Players: %d\nDay: %d\n%s"
		...
		if (m_ScreenChange) {
			...
			m_apMetaButton[SAVENOW]->SetText("保存"); //Save
			m_ScreenChange = false;
		}
		...
		// Overwriting is an option
			...
			if (m_apMetaButton[SAVENOW]->GetVisible() && m_apMetaButton[SAVENOW]->GetText() != "保存") { //Save
				m_apMetaButton[SAVENOW]->SetText(m_BlinkTimer.AlternateReal(333) ? "确定 ?" : ""); //CONFIRM?
	...
	// No game yet, show the new game menu
		...
		m_pPhaseLabel->SetText("游戏未开始"); //Game Not Started
		if (m_ScreenChange) {
		...
		m_pPhaseLabel->SetText("游戏介绍"); //Game Intro
		...
		m_apMetaButton[CONTINUE]->SetText(m_BlinkTimer.AlternateReal(333) ? "> 开始 <" : "开始"); //"> Start <" : "Start"
		...
		m_pPhaseLabel->SetText("发现了新站点"); //New Sites Found
		m_apMetaButton[CONTINUE]->SetText("跳过"); // Skip
		UpdateSiteRevealing();
		...
		m_pPhaseLabel->SetText("计算收入"); //Counting Incomes
		m_apMetaButton[CONTINUE]->SetText("跳过"); //Skip
		...
		if (g_MetaMan.m_Players[metaPlayer].IsHuman() && !g_MetaMan.m_Players[metaPlayer].IsGameOverByRound(g_MetaMan.m_CurrentRound)) {
			if (g_MetaMan.m_StateChanged) {
				...
				if (g_MetaMan.GetTotalBrainCountOfPlayer(metaPlayer) <= 0) {
					...
					m_pGameMessageLabel->SetText(g_MetaMan.m_Players[metaPlayer].GetName() + "的主脑用光了 , 他什么也做不了了 . 但是 , 你已经很努力了 !"); //'s brains are all gone, so he/she can do nothing more. Good effort, though!
		...
		if (m_PreTurn) {
			// Blink the start button to draw attention to it
			m_apMetaButton[CONTINUE]->SetText(m_BlinkTimer.AlternateReal(333) ? "> 开始回合 <" : "开始回合"); //"> Start Turn <" : "Start Turn"
		} else {
			// Dead player's turn, just waiting for the continue button to be pressed
			if (g_MetaMan.GetTotalBrainCountOfPlayer(metaPlayer) <= 0) {
				m_apMetaButton[CONTINUE]->SetText(m_BlinkTimer.AlternateReal(333) ? "> 继续 <" : "继续"); //"> Continue <" : "Continue"
				UpdatePlayerActionLines(metaPlayer);
			}
			// Normal turn
			else {
				m_apMetaButton[CONTINUE]->SetText("结束回合"); //End Turn
		...
		m_apMetaButton[CONTINUE]->SetText("开始!"); //Start!
		UpdateOffensives();
	} else if (g_MetaMan.m_GameState == MetaMan::ENDROUND) {
		m_pPhaseLabel->SetText("结束一天"); //End of Day
		m_apMetaButton[CONTINUE]->SetText("继续"); //Continue
	} else if (g_MetaMan.m_GameState == MetaMan::GAMEOVER) {
		if (g_MetaMan.m_StateChanged) {
			m_pPhaseLabel->SetText("游戏结束"); //Game Over
			m_apMetaButton[CONTINUE]->SetText("打得好 !"); //Good Game!
		...
		if (g_MetaMan.NoBrainsLeftInAnyPool())
			m_pGameMessageLabel->SetText("所有玩家的主脑都已被部署 , 因此拥有最多站点\n的队伍 ( 若同样多 , 则比较剩余资金 ) 赢得了这颗星球的开发合同 !"); //All players' brains have been deployed, and so the team with the most\nowned sites (and if tied, the most gold) won the mining contract for this planet!
		else
			m_pGameMessageLabel->SetText("领先 ( 比较占领的站点 - 若站点数相同则比较资金 )\n且仍有主脑剩余的队伍赢得了这颗星球的开发合同 !"); //The team that is in the lead (in owned sites - or gold, if tied)\nand has brains left to deploy has won the mining contract for this planet!
	}
	...
	if (!g_MetaMan.IsSuspended() && !m_PreTurn) {
		...
		if (!m_pDraggedBox && (mousePos - m_PlanetCenter).MagnitudeIsLessThan(m_PlanetRadius)) {
			...
			// 不知道是啥的站点名称汉化，先改了再说
			if (m_pHoveredScene)
				UpdateSiteNameLabel(true, CN_getNameFromDescription(m_pHoveredScene->GetDescription(), m_pHoveredScene->GetPresetName()), m_pHoveredScene->GetLocation() + m_pHoveredScene->GetLocationOffset());
	...
	while (m_pGUIController->GetEvent(&anEvent)) {
		...
		if (anEvent.GetType() == GUIEvent::Command) {
			...
			// Quit Program button pressed
				...
				HideAllScreens();
				g_MetaMan.SetSuspend(true);
				m_pConfirmationLabel->SetText("你确定你想要退出吗 ?\n任何未保存的进度\n将会丢失 !"); //"Sure you want to quit to OS?\nAny unsaved progress\nwill be lost!"
				m_pConfirmationButton->SetText("退出"); //Quit
			...
			if (anEvent.GetControl() == m_apMetaButton[CONFIRM]) {
				...
				if (m_pConfirmationButton->GetText() == "退出") //Quit
					System::SetQuit();
			...
			for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
				if (anEvent.GetControl() == m_apPlayerControlButton[player]) {
					...
					if (m_apPlayerControlButton[player]->GetText() == "None")
						...
						m_apPlayerControlButton[player]->SetText("玩家"); //Human
					else
			...
			if (eventControlName == "StartButton") {
				...
				if (!g_MetaMan.GameIsSaved()) {
					...
					m_pConfirmationButton->SetText("开始新游戏"); //Start New
					m_pConfirmationBox->SetVisible(true);
				} else
					StartNewGame();
			...
			if (eventControlName == "SaveButton") {
				...
				if (!m_NewSaveBox->GetText().empty() || m_apMetaButton[SAVENOW]->GetText() != "保存") { //Save
					...
					m_apMetaButton[SAVENOW]->SetText("确定 ?"); //CONFIRM?
					g_GUISound.ItemChangeSound()->Play();
				}
			}
			...
			if (anEvent.GetControl() == m_apMetaButton[SCENEACTION] && m_pSelectedScene) {
				...
				if (g_MetaMan.GetRemainingFundsOfPlayer(metaPlayer, 0, false, false) < SCANCOST) {
					m_apMetaButton[SCENEACTION]->SetText("你的资金储备不足 !"); //NOT ENOUGH FUNDS!
...
void MetagameGUI::CompletedActivity() {
	...
	if (pDoneActivity && pAlteredScene && m_pPlayingScene) {
		...
		// If a MetaFight activity isn't over yet, that means we're just pausing and will resume
			...
			m_AnimTimer2.Reset();
			m_apMetaButton[CONTINUE]->SetText("恢复"); //Resume
...
void MetagameGUI::UpdateSiteRevealing() {
	...
	m_pPhaseLabel->SetText((m_AnimCountEnd - m_AnimCountStart) > 1 ? "已发现黄金站点 !" : "已发现黄金站点 !"); //"Gold Sites Found!" : "Gold Site Found!"
	...
	if (m_NewSiteIndicators.size() == m_AnimCountEnd - m_AnimCountStart) {
		m_apMetaButton[CONTINUE]->SetText(m_AnimTimer2.AlternateReal(333) ? "> 继续 <" : "继续"); //"> Continue <" : "Continue"
	}
...
void MetagameGUI::UpdateBaseBuilding() {
	m_pPhaseLabel->SetText("建造基地中"); //Building Bases
		...
		// Shrink each meter from its current value to 0
			...
			if (NewAnimMode()) {
				...
				if (m_AnimBuildCount == 0)
					PlayerTextIndication(m_AnimMetaPlayer, m_AnimFundsMax < 100 ? "买不起 !" : "没什么可建造的 !", m_PlanetCenter + m_ActionSiteLines[m_AnimMetaPlayer][m_AnimActionLine].m_PlanetPoint, 1500); //"Can't afford any!" : "Nothing to build!"
				else {
					char str[64];
					std::snprintf(str, sizeof(str), m_AnimBuildCount == 1 ? "已建造 %d 个项目" : "已建造 %d 个项目", m_AnimBuildCount); //"Built %d item" : "Built %d items"
...
void MetagameGUI::UpdateOffensives() {
	...
	if (m_AnimActivityChange) {
		...
		std::snprintf(str, sizeof(str), "第 %d 场战斗 共 %d 场", (g_MetaMan.m_CurrentOffensive + 1), (int)(g_MetaMan.m_RoundOffensives.size())); //"Battle %d of %d"
	...
	if (m_AnimMode == LINECONNECTFW) {
		...
		// 不知道是啥的标题汉化，还是先改了再说
		UpdateSiteNameLabel(true, CN_getNameFromDescription(m_pAnimScene->GetDescription(), m_pAnimScene->GetPresetName()), m_pAnimScene->GetLocation() + m_pAnimScene->GetLocationOffset(), 1.8);
		...
		if (m_BattleToResume)
			m_apMetaButton[CONTINUE]->SetText(m_AnimTimer2.AlternateReal(333) ? "> 恢复 <" : "恢复"); //"> Resume <" : "Resume"
		else
			m_apMetaButton[CONTINUE]->SetText(m_AnimTimer2.AlternateReal(333) ? "> 开始 ! <" : "开始 !"); //"> Start! <" : "Start!"
	...
	if (m_AnimMode == SHOWPOSTBATTLEPAUSE) {
		...
		m_apMetaButton[CONTINUE]->SetText("继续"); //Next
...
void MetagameGUI::UpdatePreBattleAttackers(float progress) {
	...
	for (std::vector<MetaPlayer>::iterator mpItr = g_MetaMan.m_Players.begin(); mpItr != g_MetaMan.m_Players.end(); ++mpItr) {
		...
		if (g_MetaMan.m_RoundOffensives[g_MetaMan.m_CurrentOffensive]->PlayerActive((*mpItr).GetInGamePlayer()) &&
			...
			if (progress < 1.0)
				std::snprintf(str, sizeof(str), "%c%c", -48, 127);
		...
		if (g_MetaMan.m_RoundOffensives[g_MetaMan.m_CurrentOffensive]->PlayerHadBrain((*mpItr).GetInGamePlayer()) &&
			...
			if (!m_pAnimScene->GetResidentBrain((*mpItr).GetInGamePlayer())) {
				...
				if (progress <= 0) {
					// Death mask
					if (progress > 0)
						std::snprintf(str, sizeof(str), "%c%c", -26, 127);
			...
			// The SURVIVORS who aren't evacuating either - ie they won and are staying
				...
				m_apPlayerBrainTravelLabel[mp]->SetText(str);
				m_apPlayerBrainTravelLabel[mp]->SetToolTip("赢 下 这 个 站 点 并 在 此 扎 根 的 新 的 主 脑 定 居 者 ."); //The new resident brain that has won this site and is settling in here now.
...
void MetagameGUI::UpdateScenesBox(bool sceneChanged) {
	...
	if (m_pSelectedScene && !g_MetaMan.IsSuspended()) {
		...
		std::snprintf(str, sizeof(str), "%c%c", -36, 127);
		m_pSceneCloseButton->SetText(std::string(str));

		// Set the currently selected scene's texts
		m_pSceneInfoPopup->SetVisible(true);
		// MetaGame里场景描述的中文标题显示，如果有的话
		m_pSceneNameLabel->SetText(CN_getNameFromDescription(m_pSelectedScene->GetDescription(), m_pSelectedScene->GetPresetName()));
			...
			// Not owned by this player's team, so set up the scanning controls
				...
				if (sceneChanged) {
					...
					if (m_pSelectedScene->IsScanScheduled(team)) {
						...
						m_apMetaButton[SCANLATER]->SetVisible(false);
						m_pScanInfoLabel->SetVisible(true);
						m_pScanInfoLabel->SetText("- 轨道扫描已预定 -"); //"- Orbital scan scheduled -"
					}
				...
				if (g_MetaMan.m_Players[metaPlayer].GetBrainPoolCount() <= 0) {
					m_pSceneBudgetLabel->SetText("- 没有主脑可部署 -");
					m_pSceneBudgetLabel->SetToolTip("由 于 你 的 主 脑 池 中 没 有 更 多 的 主 脑 可 供 部 署 , 你 无 法 攻 击 这 个 或 任 何 其 他 站 点 . 保 护 你 拥 有 的 站 点 , 希 望 你 能 保 存 足 够 的 主 脑 来 赢 得 比 赛 !");
					m_pSceneBudgetBar->SetToolTip("由 于 你 的 主 脑 池 中 没 有 更 多 的 主 脑 可 供 部 署 , 你 无 法 攻 击 这 个 或 任 何 其 他 站 点 . 保 护 你 拥 有 的 站 点 , 希 望 你 能 保 存 足 够 的 主 脑 来 赢 得 比 赛 !");
					/*
...
void MetagameGUI::UpdateGameSizeLabels() {
	...
	char str[256];
	std::snprintf(str, sizeof(str), "游戏尺寸 : %d/%d 站点", m_pSizeSlider->GetValue(), totalCount); //"Game Size: %d/%d sites"
	...
	std::snprintf(str, sizeof(str), "游戏长度 : %c%c%c%c%d 初始主脑", -48, 127, -36, 127, brainCount); //"Game Length: %c%c%d starting brains"
...
void MetagameGUI::UpdatePlayerSetup() {
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		if (m_apPlayerControlButton[player]->GetText() == "None") {
			...
			totalPlayers++;
			if (m_apPlayerControlButton[player]->GetText() == "玩家") //Human
				humanPlayers++;
	...
	if (humanPlayers < 1)
		m_pErrorLabel->SetText("需要 1 名玩家"); //Need 1 Human
	else if (totalPlayers < 2)
		m_pErrorLabel->SetText("需要 2 名玩家"); //Need 2 Players
	else if (teamList.size() < 2)
		m_pErrorLabel->SetText("需要 2 支队伍"); //Need 2 Teams
	else
...
void MetagameGUI::UpdatePlayerBars() {
	if (g_MetaMan.m_GameState >= MetaMan::NOGAME && g_MetaMan.m_GameState <= MetaMan::ENDROUND && !g_MetaMan.IsSuspended()) {
		...
		for (std::vector<MetaPlayer>::iterator mpItr = g_MetaMan.m_Players.begin(); mpItr != g_MetaMan.m_Players.end(); ++mpItr) {
			...
			std::snprintf(str, sizeof(str), "%c%c%c%c%d", brainDisplayCount > 0 ? -48 : -25, 127, -36, 127, brainDisplayCount);
...
void MetagameGUI::BrainsChangeIndication(int metaPlayer, int change, const Vector& screenPos, int fontAlignment, double animLengthMS) {
	...
	// [Brain Icon] [X] Number
	// The number to display is adjusted with whether any brains are out and about in the gui animations
	std::snprintf(str, sizeof(str), change >= 0 ? "%c%c+%d" : "%c%c%d", -48, 127, change);
