	...
	switch (m_IntroSequenceState) {
		...
		case IntroSequence::ShowSlide1:
			...
			if (m_SectionElapsedTime > 1.25F) {
				m_SlideshowSlideText = "在人类最黑暗的世纪末..."; //At the end of humanity's darkest century...
			}
			break;
		case IntroSequence::ShowSlide2:
			...
			if (m_SectionElapsedTime < m_SectionDuration - 1.75F) {
				m_SlideshowSlideText = "...一种奇特的共生关系出现在了人与机器之间 ."; //...a curious symbiosis between man and machine emerged
			}
			break;
		...
		case IntroSequence::ShowSlide5:
			...
			m_SlideshowSlideText = "人类接触到了其他文明..."; //Other civilizations were encountered...
			break;
		case IntroSequence::ShowSlide6:
			...
			m_SlideshowSlideText = "...和平的星际贸易很快被建立了 ."; //...and peaceful intragalactic trade soon established.
			break;
		case IntroSequence::ShowSlide7:
			...
			m_SlideshowSlideText = "现在 , 不停发展的文明产生了对资源的大量需求..."; //Now, the growing civilizations create a huge demand for resources...
			break;
		case IntroSequence::ShowSlide8:
			...
			if (m_SectionProgress < 0.3F) {
				m_SlideshowSlideText = "...这种需求只能通过不断扩张得到满足 ."; //...which can only be satisfied by the ever-expanding frontier.
			} else if (m_SectionProgress > 0.33F && m_SectionProgress < 0.64F) {
				m_SlideshowSlideText = "竞争是残酷的 , 在这场银河淘金热中 , 一切皆有可能."; //Competition is brutal and anything goes in this galactic gold rush.
			} else if (m_SectionProgress > 0.67F) {
				m_SlideshowSlideText = "你现在也将冒险前往一颗未开发的星球..."; //You will now join it on a venture to an untapped planet...
			}
			break;
