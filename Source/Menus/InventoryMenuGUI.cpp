...
void InventoryMenuGUI::UpdateFullModeEquippedItemButtons() {
	...
	if (m_GUISelectedItem && m_GUISelectedItem->Button == m_GUIEquippedItemButton && m_GUISelectedItem->DragWasHeldForLongEnough()) {
		...
		m_GUIEquippedItemButton->SetIconAndText(equippedItem->GetGraphicalIcon(), CN_getNameFromDescription(equippedItem->GetDescription(), equippedItem->GetPresetName()));
	} else {
	...
	if (m_GUISelectedItem && m_GUISelectedItem->Button == m_GUIOffhandEquippedItemButton && m_GUISelectedItem->DragWasHeldForLongEnough()) {
		...
		m_GUIOffhandEquippedItemButton->SetIconAndText(offhandEquippedItem->GetGraphicalIcon(), CN_getNameFromDescription(offhandEquippedItem->GetDescription(), offhandEquippedItem->GetPresetName()));
	} else {
	...
	for (int i = startIndex; (i - startIndex) < c_FullViewPageItemLimit; i++) {
		...
		if (i <= lastPopulatedIndex) {
			...
			if (m_GUISelectedItem && m_GUISelectedItem->Button == itemButton && m_GUISelectedItem->DragWasHeldForLongEnough()) {
				...
				itemButton->SetIconAndText(inventoryItem->GetGraphicalIcon(), CN_getNameFromDescription(inventoryItem->GetDescription(), inventoryItem->GetPresetName()));
			}
...
void InventoryMenuGUI::UpdateFullModeInformationText(const std::deque<MovableObject*>* inventory) {
	if (!m_GUIShowInformationText && m_GUIInformationText->GetVisible()) {
		...
		if (m_GUISelectedItem) {
			if (m_GUISelectedItem->DragWasHeldForLongEnough()) {
				informationText = "拖 拽 到 其 他 物 品 上 以 交 换 位 置 , "; //Drag this item to another one to swap places with it, 
				informationText += m_GUIReloadButton->GetEnabled() ? "拖 拽 到 换 弹 按 钮 以 换 弹 , " : ""; //"the reload button to reload it, " : ""
				informationText += "拖 拽 到 丢 弃 按 钮 或 存 储 窗 口 外 以 丢 弃 ."; //or the drop button to drop it. Drag it out of the inventory window to toss it.
			} else {
				informationText = m_MenuController->IsMouseControlled() ? "点 击 另 一 个 物 品 进 行 交 换 , " : "点 击 另 一 个 物 品 进 行 交 换 , "; //"Click another item to swap places with it, " : "Press another item to swap places with it, "
				informationText += m_GUIReloadButton->GetEnabled() ? "点 击 换 弹 按 钮 以 换 弹 , " : ""; //"the reload button to reload it, " : ""
				informationText += m_MenuController->IsMouseControlled() ? "点 击 丢 弃 按 钮 " : "按 下 丢 弃 按 钮 "; //"or the drop button " : "or press the drop button/use the drop key "
				informationText += "以 丢 弃 它 ."; //to drop it.
			}
		} else {
			if (m_GUIDisplayOnly) {
				...
				informationText = m_MenuController->IsMouseControlled() ? "鼠 标 点 击 一 个 物 品 进 行 互 动 . 你 也 可 以 按 住 并 拖 拽 物 品 ." : " 按 下 一 个 物 品 去 与 它 互 动 ."; //"Click an item to interact with it. You can also click and hold to drag items." : "Press an item to interact with it."
			}
		}
