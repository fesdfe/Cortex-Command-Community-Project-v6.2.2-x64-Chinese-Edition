...
void SettingsVideoGUI::SetEnabled(bool enable) const {
	...
	if (enable) {
		...
		m_CustomResolutionMultiplierComboBox->SetText(std::to_string(m_NewResMultiplier));
#endif
		std::string windowedText = g_WindowMan.IsFullscreen() ? "窗口化" : "已是窗口化"; //"Windowed" : "Scale To Window"
		m_ResolutionQuickToggleButtons[ResolutionQuickChangeType::Windowed]->SetText(windowedText);
		std::string fullscreenText = g_WindowMan.IsFullscreen() ? "已是全屏" : "全屏"; //"Scale To Fullscreen" : "Fullscreen"
...
void SettingsVideoGUI::ApplyCustomResolution() {
	...
	if (exitCode == EOF || newResMultiplier == 0) {
		m_CustomResolutionMessageLabel->SetText("分辨率倍数不是一个有效数字 !"); //Resolution Multiplier is not a valid number!
	...
	if (m_NewResX < c_MinResX || m_NewResY < c_MinResY) {
		m_CustomResolutionMessageLabel->SetText("横向或纵向分辨率过小 ( 最小分辨率 : " + std::to_string(c_MinResX) + "x" + std::to_string(c_MinResY) + ") ."); //"Resolution width or height lower than the minimum ("  ") is not supported."
