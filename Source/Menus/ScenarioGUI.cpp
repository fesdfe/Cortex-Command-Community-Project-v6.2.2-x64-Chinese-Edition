...
void ScenarioGUI::SetSelectedActivity(const Activity* newSelectedActivity) {
	...
	if (m_SelectedActivity) {
		...
		if (m_ActivityScenes && m_ActivityScenes->size() == 1) {
			m_ActivityDescriptionLabel->SetText(m_SelectedActivity->GetDescription() + "\n\n该 活 动 只 有 一个 可 用 站 点 , 已 自 动 选 择 该 站 点 ."); //The only site where this activity can be played has been selected for you.
		} else if (m_ActivityScenes && m_ActivityScenes->size() > 1) {
			m_ActivityDescriptionLabel->SetText(m_SelectedActivity->GetDescription() + "\n\n该 活 动 的 可 用 站 点 已 被 标 记 在 星 球 上 . 选 择 一 个 以 开 始 !"); //Sites where this activity can be played appear on the planet. Select one to begin!
		} else {
			m_ActivityDescriptionLabel->SetText(m_SelectedActivity->GetDescription() + "\n\n该 活 动 没 有 兼 容 的 可 用 站 点 ! 请 选 择 其 他 活 动 ."); //No sites appear to be compatible with this selected activity! Please try another.
		}
		SetSelectedScene((m_ActivityScenes && m_ActivityScenes->size() == 1) ? m_ActivityScenes->front() : nullptr);
	} else {
		m_ActivityDescriptionLabel->SetText("未 选 择 活 动 ."); //No Activity selected.
	}
...
void ScenarioGUI::SetSelectedScene(Scene* newSelectedScene) {
	...
	if (m_SelectedScene) {
		...
		// 站点名称汉化
		m_SceneNameLabel->SetText(CN_getNameFromDescription(m_SelectedScene->GetDescription(), m_SelectedScene->GetPresetName()));

		Vector sceneSizeMeters = m_SelectedScene->GetDimensions() / c_PPM;
		std::string sceneDimensions = "站 点 尺 寸 : " + std::to_string(sceneSizeMeters.GetFloorIntX()) + " x " + std::to_string(sceneSizeMeters.GetFloorIntY()) + " meters"; //"Site Dimensions: " " meters"
...
void ScenarioGUI::FetchActivitiesAndScenesLists() {
	...
	for (Entity* presetEntity: presetList) {
		if (GameActivity* presetActivity = dynamic_cast<GameActivity*>(presetEntity)) {
			...
			// 下面的东西将向活动下拉栏添加活动条目，修改这里可以改变活动条目的名称，太棒啦！
			m_ActivitySelectComboBox->AddItem(CN_getNameFromDescription(presetActivity->GetDescription(), presetActivity->GetPresetName()), "", nullptr, presetActivity);
...
ScenarioGUI::ScenarioMenuUpdateResult ScenarioGUI::Update() {
	...
	if (!m_ActivityConfigBox->IsEnabled()) {
		...
		if (m_SceneInfoBox->GetVisible()) {
			m_StartActivityConfigButton->SetText(m_BlinkTimer.AlternateReal(333) ? "在此开始" : "> 在此开始 <"); //"Start Here" : "> Start Here <"
...
void ScenarioGUI::UpdateHoveredSitePointLabel(int mouseX, int mouseY) {
	...
	if (m_ActivityScenes && !m_DraggedBox && !m_ActivityInfoBox->PointInside(mouseX, mouseY) && !m_SceneInfoBox->PointInside(mouseX, mouseY)) {
		...
		if (candidateScene && candidateScene != m_HoveredScene) {
			...
			// 站点名称汉化
			m_SitePointNameLabel->SetText(CN_getNameFromDescription(m_HoveredScene->GetDescription(), m_HoveredScene->GetPresetName()));
