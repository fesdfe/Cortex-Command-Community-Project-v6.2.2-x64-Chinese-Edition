...
int GATutorial::Create() {
	...
	m_Description = "[ 教程任务 ] 一 个 用 于 学 习 如 何 游 玩 表 层 指 挥 的 教 程 . 一 个 开 始 适 应 游 戏 的 好 地 方 !"; //"A tutorial for learning how to play Cortex Command. A good place to start!";
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		...
		if (m_ActivityState != ActivityState::Over) {
			...
			// Mark the player brain to be protected when the fight happens
				...
				AddObjectivePoint("保护!", m_Brain[player]->GetPos() + Vector(0, 10), team, GameActivity::ARROWUP); //Protect!
				// Mark the CPU brain for desctruction too
				if (g_MovableMan.IsActor(m_pCPUBrain))
					AddObjectivePoint("摧毁!", m_pCPUBrain->GetPos() + Vector(0, 12), team, GameActivity::ARROWUP); //Destroy!
			}
		}
	...
	*/
	...
	if (m_ActivityState == ActivityState::Over && m_WinnerTeam == Teams::TeamOne) {
		if (m_GameOverTimer.IsPastSimMS(m_GameOverPeriod)) {
			g_FrameMan.ClearScreenText(ScreenOfPlayer(m_TutorialPlayer));
			g_FrameMan.SetScreenText("按 [SPACE] 或 [START] 继续 !", ScreenOfPlayer(m_TutorialPlayer), 750); //Press [SPACE] or [START] to continue!
		}
...
void GATutorial::SetupAreas() {
	...
	m_TutAreaSteps[BRAINCHAMBER].push_back(TutStep("欢迎来到表层指挥", 4000, "Missions.rte/Objects/Tutorial/CCLogo.png", 2)); //Welcome to Cortex Command
	m_TutAreaSteps[BRAINCHAMBER].push_back(TutStep("上面这个是你的没躯干的脑子", 4000, "Missions.rte/Objects/Tutorial/ArrowUp.png", 2)); //Above is your disembodied brain
	m_TutAreaSteps[BRAINCHAMBER].push_back(TutStep("你可以靠它远程控制你的其他躯干", 4000, "Missions.rte/Objects/Tutorial/BodyZap.png", 2)); //With it, you can remotely control other bodies
	m_TutAreaSteps[BRAINCHAMBER].push_back(TutStep("通过按下 [" + MAPNAME(INPUT_PREV) + "] 或 [" + MAPNAME(INPUT_NEXT) + "] 切换控制的躯干", 4000, "Missions.rte/Objects/Tutorial/BodyZap.png", 2)); //"Switch to one now by using [" + MAPNAME(INPUT_PREV) + "] or ["
	m_TutAreaSteps[BRAINCHAMBER].push_back(TutStep("如果你没有设置好你的按键映射，先按 [Esc] 去改设置吧", 8000, "Missions.rte/Objects/Tutorial/Joystick.png", 2)); //If you haven't set up your controls yet, hit [Esc] and do so
	...
	m_TutAreaSteps[BODYSTORAGE].push_back(TutStep("这里有些笨笨躯干供你练习", 4000, "Missions.rte/Objects/Tutorial/BodyHop.png", 2)); //Here are some dummy bodies for practice
	m_TutAreaSteps[BODYSTORAGE].push_back(TutStep("通过按 [" + MAPNAME(INPUT_PREV) + "] 和 [" + MAPNAME(INPUT_NEXT) + "] 在他们之间快速左右切换", 6000, "Missions.rte/Objects/Tutorial/BodyZap.png", 2)); //"Quickly switch control between them left and right with [" + MAPNAME(INPUT_PREV) + "] and [" + MAPNAME(INPUT_NEXT) + "]"
	m_TutAreaSteps[BODYSTORAGE].push_back(TutStep("或者长按 [" + MAPNAME(INPUT_PREV) + "] 或 [" + MAPNAME(INPUT_NEXT) + "] 然后用鼠标选择控制", 6000, "Missions.rte/Objects/Tutorial/BodyZap.png", 2)); //"Or hold down either [" + MAPNAME(INPUT_PREV) + "] or [" + MAPNAME(INPUT_NEXT) + "] to get a selection cursor"
	m_TutAreaSteps[BODYSTORAGE].push_back(TutStep("切换到最左边的躯干然后按 [" + MAPNAME(INPUT_L_LEFT) + "] 走出房间", 8000, "Missions.rte/Objects/Tutorial/ArrowLeft.png", 2)); //("Switch to the leftmost body and walk it out of the room with [" + MAPNAME(INPUT_L_LEFT) + "]"
	...
	m_TutAreaSteps[SHAFT].push_back(TutStep("按 [" + JumpName + "] 去激活喷气背包", 4000, "Missions.rte/Objects/Tutorial/ArrowUp.png", 2)); //"Use [" + JumpName + "] to activate jetpack"
	m_TutAreaSteps[SHAFT].push_back(TutStep("多次少量的启动喷气背包可以更好的控制它", 4000, "Missions.rte/Objects/Tutorial/BodyJetpack.png", 2)); //"Fire jetpack in bursts for better control"
	m_TutAreaSteps[SHAFT].push_back(TutStep("通过改变躯干朝向来调整喷气背包推进方向", 4000, "Missions.rte/Objects/Tutorial/BodyJetpack.png", 2)); //Adjust the jet direction by aiming or looking
	m_TutAreaSteps[SHAFT].push_back(TutStep("飞行高度取决于你躯干的总重量", 4000, "Missions.rte/Objects/Tutorial/BodyJetpack.png", 2)); //Jump height is affected by the body's total weight
	m_TutAreaSteps[SHAFT].push_back(TutStep("所以你带的东西越多 , 你就飞的越少", 8000, "Missions.rte/Objects/Tutorial/BodyJetpack.png", 2)); //So the more you carry, the less you can jump
	...
	m_TutAreaSteps[OBSTACLECOURSE].push_back(TutStep("障碍课程", 2000, "Missions.rte/Objects/Tutorial/BodyHop.png", 2)); //Obstacle course
	m_TutAreaSteps[OBSTACLECOURSE].push_back(TutStep("通过按下 [" + MAPNAME(INPUT_L_RIGHT) + "] 攀爬障碍物", 4000, "Missions.rte/Objects/Tutorial/ArrowRight.png", 2)); //Climb obstacles by holding [
	m_TutAreaSteps[OBSTACLECOURSE].push_back(TutStep("简单的走向梯子你就能爬上它们", 8000, "Missions.rte/Objects/Tutorial/BodyClimb.png", 2)); //Even climb ladders by simply moving toward them
	m_TutAreaSteps[OBSTACLECOURSE].push_back(TutStep("你也可以按下 [" + CrouchName + "] 来蹲下爬行", 6000, "Missions.rte/Objects/Tutorial/BodyCrawl.png", 2)); //You can also crouch and crawl with [
	m_TutAreaSteps[OBSTACLECOURSE].push_back(TutStep("在狭小的空间里 , 你也许需要把头低下才能通过", 8000, "Missions.rte/Objects/Tutorial/BodyCrawl.png", 2)); //In tight spaces, you may need to angle your head down to get through
	...
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("靶场", 2000, "Missions.rte/Objects/Tutorial/FireTarget.png", 1)); //Firing range
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("捡起一把武器需要先站在武器旁边", 4000, "Missions.rte/Objects/Tutorial/ArrowRight.png", 2)); //Pick up the weapon by first standing over it
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("然后按住 [" + PieName + "]", 4000, "Missions.rte/Objects/Tutorial/MenuPickUp.png", 1, 500)); //And then hold down
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("聚焦 '捡拾' 按钮", 4000, "Missions.rte/Objects/Tutorial/MenuPickUp.png", 2, 500)); //Point up to 'Pick Up'
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("松开 [" + PieName + "] 以执行聚焦的指令", 4000, "Missions.rte/Objects/Tutorial/MenuPickUp.png", 1)); //"Release [" + PieName + "] to complete the command"
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("如果你持续瞄准敌人", 4000, "Missions.rte/Objects/Tutorial/BodyAim.png", 3)); //If you aim continuously toward your target
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("那么你的精度和视距就会增大", 4000, "Missions.rte/Objects/Tutorial/BodyAim.png", 3)); //It improves your accuracy and view distance
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("按下 [" + FireName + "] 开火 !", 4000, "Missions.rte/Objects/Tutorial/BodyFire.png", 2)); //"Use [" + FireName + "] to Fire!"
	m_TutAreaSteps[FIRINGRANGE].push_back(TutStep("通过 [" + PieName + "] + [向上] 手动装弹", 8000, "Missions.rte/Objects/Tutorial/BodyFire.png", 2)); //"Reload manually with [" + PieName + "] + up"
	...
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("捡起这儿的挖掘工具", 4000, "Missions.rte/Objects/Tutorial/MenuPickUp.png", 2, 500)); //Pick up the digging tool
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("用它把这儿的土挖了", 8000, "Missions.rte/Objects/Tutorial/DigPile.png", 2, 750)); //Use it on the dirt here
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("如果你挖到了金子 , 你的团队的资金就会上涨", 4000, "Missions.rte/Objects/Tutorial/Funds.png", 2, 250)); //If you dig up gold, it is added to your team's funds
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("你可以在购买菜单花钱买东西", 4000, "Missions.rte/Objects/Tutorial/Funds.png", 1, 333)); //Funds can be spent in the Buy Menu
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("购买菜单通过环形菜单打开 , 环形菜单你已经见过了", 4000, "Missions.rte/Objects/Tutorial/MenuBuyMenu.png", 1, 500)); //Which is opened through the Command Menu
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("按住 [" + PieName + "] 打开环形菜单并聚焦 '购买菜单'", 6000, "Missions.rte/Objects/Tutorial/MenuBuyMenu.png", 2, 500)); //"Hold [" + PieName + "] and point up and left to 'Buy Menu'"
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("在购买菜单买东西就像把东西加入购物车一样", 6000, "Missions.rte/Objects/Tutorial/BuyMenuCargo.png", 1, 500)); //The Buy Menu works like a shopping cart
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("把你想买的东西加入到荷载明细", 6000, "Missions.rte/Objects/Tutorial/BuyMenuCargo.png", 2, 500)); //Add to the Cargo list the items you want delivered
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("然后点 BUY 按钮 , 或者点到菜单外面", 4000, "Missions.rte/Objects/Tutorial/BuyMenuBuy.png", 2, 500)); //Then use the BUY button, or click outside the menu
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("最后选一个平坦点的地方空投你的货物", 8000, "Missions.rte/Objects/Tutorial/BuyMenuBuy.png", 1, 500)); //Finally select a flat area where you want the goods delivered
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("现在，你可以去西边自由地尝试飞行和攀爬", 6000, "Missions.rte/Objects/Tutorial/ArrowLeft.png", 2)); //Next, you can go explore to the west to try flying and climbing in the wild
	m_TutAreaSteps[ROOFTOP].push_back(TutStep("或者去东边学一下编队!", 8000, "Missions.rte/Objects/Tutorial/ArrowRight.png", 2)); //Or, go to the east to learn about squads!
