	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		...
		if (m_pEditorGUI[player]->GetEditorGUIMode() == SceneEditorGUI::DONEEDITING) {
			...
			// Ready to start
				...
				g_FrameMan.ClearScreenText(ScreenOfPlayer(player));
				g_FrameMan.SetScreenText("已就绪 - 正在等待其他玩家 ...", ScreenOfPlayer(player), 333); //READY to start - wait for others to finish...
...
void GameActivity::Update() {
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		...
		// Manually selecting a new actor to switch to
			...
			g_FrameMan.SetScreenText("选择一个躯干以获取其控制权 ...", ScreenOfPlayer(player)); //Select a body to switch control to...
			// Get cursor input
		...
		// Selecting points on the scene for the AI to go to
			...
			// Continuously display message
			g_FrameMan.SetScreenText("设置你希望AI控制其前往的路径点 ...", ScreenOfPlayer(player)); //Set waypoints for the AI to go to...
			// Get cursor input
			m_PlayerController[player].RelativeCursorMovement(m_ActorCursor[player]);
			...
			// Continuously display message
			g_FrameMan.SetScreenText("圈选单位以编队 ...", ScreenOfPlayer(player)); //Select units to group...
		...
		// Selecting LZ, a place for the craft to land
			...
			if (m_PlayerController[player].IsState(PRESS_SECONDARY) && m_ActivityState != ActivityState::PreGame) {
				...
				g_FrameMan.SetScreenText("订单取消 !", ScreenOfPlayer(player), 333); //Order canceled!
				m_MessageTimer[player].Reset();
				g_GUISound.UserErrorSound()->Play(player);
				// Flash the same actor, jsut to show the control went back to him
				if (m_ControlledActor[player] && m_ControlledActor[player]->GetPieMenu()) {
		...
		// Deathwatching
			...
			g_FrameMan.SetScreenText("控制丢失 !", ScreenOfPlayer(player)); //Lost control of remote body!
		...
		if (m_ActivityState == ActivityState::Over && m_GameOverTimer.IsPastRealMS(m_GameOverPeriod)) {
			...
			if (g_FrameMan.IsInMultiplayerMode())
				g_FrameMan.SetScreenText("所有玩家必须按住并按下 [BACKSPACE] 以继续 !", ScreenOfPlayer(player), 750); //All players must press and hold [BACKSPACE] to continue!
			else
				g_FrameMan.SetScreenText("按下 [SPACE] 或 [START] 以继续 !", ScreenOfPlayer(player), 750); //Press [SPACE] or [START] to continue!
	...
	for (int team = Teams::TeamOne; team < Teams::MaxTeamCount; ++team) {
		...
		if (!m_Deliveries[team].empty()) {
			...
			if (m_MessageTimer[player].IsPastSimMS(1000)) {
				char message[512];
				std::snprintf(message, sizeof(message), "下一波投送将在 %i 秒后抵达", ((int)m_Deliveries[team].front().delay - (int)m_Deliveries[team].front().timer.GetElapsedSimTimeMS()) / 1000); //"Next delivery in %i secs"
		...
		if (!m_Deliveries[team].empty() && m_Deliveries[team].front().timer.IsPastSimMS(m_Deliveries[team].front().delay)) {
			...
			int player = m_Deliveries[team].front().orderedByPlayer;
			if (pDeliveryCraft) {
				g_FrameMan.SetScreenText("投送已抵达 !", ScreenOfPlayer(player), 333); //"Your order has arrived!"
				m_MessageTimer[player].Reset();
...
void GameActivity::DrawGUI(BITMAP* pTargetBitmap, const Vector& targetPos, int which) {
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		...
		if (m_ViewState[player] == ViewState::LandingZoneSelect) {
			...
			// Text
			pSmallFont->DrawAligned(&pBitmapInt, landZone.m_X, landZone.m_Y - 50, m_AIReturnCraft[player] ? "投 送 至 此" : "降 落 在 这", GUIFont::Centre); //landZone.m_Y - 42, "Deliver here" : "Travel here"
			pSmallFont->DrawAligned(&pBitmapInt, landZone.m_X, landZone.m_Y - 40, "然 后", GUIFont::Centre); //landZone.m_Y - 36, "and then"
			pLargeFont->DrawAligned(&pBitmapInt, landZone.m_X, landZone.m_Y - 30, m_AIReturnCraft[player] ? "返回" : "待命", GUIFont::Centre); //"RETURN" : "STAY"
			...
			if (targetPos.IsZero() && (landZone.m_X < halfWidth || landZone.m_X > g_SceneMan.GetSceneWidth() - halfWidth)) {
				...
				// Text
				pSmallFont->DrawAligned(&pBitmapInt, wrappedX, landZone.m_Y - 50, m_AIReturnCraft[player] ? "投 送 至 此" : "降 落 在 这", GUIFont::Centre); //landZone.m_Y - 42, "Deliver here" : "Travel here"
				pSmallFont->DrawAligned(&pBitmapInt, wrappedX, landZone.m_Y - 40, "然 后", GUIFont::Centre); //landZone.m_Y - 36, "and then"
				pLargeFont->DrawAligned(&pBitmapInt, wrappedX, landZone.m_Y - 30, m_AIReturnCraft[player] ? "返回" : "待命", GUIFont::Centre); //"RETURN" : "STAY"
			}
		}
	}
