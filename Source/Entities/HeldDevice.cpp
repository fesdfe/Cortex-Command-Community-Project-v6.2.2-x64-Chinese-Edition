		...
		if (m_Parent) {
			...
			if (pSymbolFont && pTextFont) {
				...
				if (!m_SeenByPlayer[viewingPlayer]) {
					...
					pSymbolFont->DrawAligned(&targetAllegroBitmap, drawPos.GetFloorIntX() - 1, drawPos.GetFloorIntY() - 20, pickupArrowString, GUIFont::Centre, 0, 0, 0, 1);

					//改这个地方可以改变掉落在地上的物品所显示的名字
					//pTextFont->DrawAligned(&targetAllegroBitmap, drawPos.GetFloorIntX(), drawPos.GetFloorIntY() - 29, m_PresetName, GUIFont::Centre);
					std::string cnName = "";
					if (m_PresetDescription.length()>2)
						if (m_PresetDescription.at(0) == '[') {
							if (m_PresetDescription.find(']') != -1)
								cnName = m_PresetDescription.substr(1, m_PresetDescription.find(']') - 1);
							else
								cnName = m_PresetName;
						} else
							cnName = m_PresetName;
						...
						cnName = m_PresetName;
					pTextFont->DrawAligned(&targetAllegroBitmap, drawPos.GetFloorIntX(), drawPos.GetFloorIntY() - 29, cnName, GUIFont::Centre);
				}
			}
		}
	}
