	...
	if (pSmallFont && pSymbolFont) {
		...
		if (!m_Controller.IsState(PIE_MENU_ACTIVE) || actorScreen != whichScreen) {
			...
			if (m_Health > 0) {
				...
				str[0] = -39;
				str[1] = 0;
				pSymbolFont->DrawAligned(&bitmapInt, drawPos.m_X - 10, drawPos.m_Y + m_HUDStack, str, GUIFont::Left, 0, 0, 0, 1);
			}

			std::snprintf(str, sizeof(str), "%.0f", std::ceil(m_Health));
			pSymbolFont->DrawAligned(&bitmapInt, drawPos.m_X - 0, drawPos.m_Y + m_HUDStack, str, GUIFont::Left, 0, 0, 0, 1);
