	...
	if (m_Controller.IsPlayerControlled() && g_ActivityMan.GetActivity()->ScreenOfPlayer(m_Controller.GetPlayer()) == whichScreen && pSmallFont && pSymbolFont) {
		...
		if (m_pFGArm || m_pBGArm) {
			...
			if (fgHeldFirearm || bgHeldFirearm) {
				...
				std::string fgWeaponString = "空"; //EMPTY
				if (fgHeldFirearm) {
					if (fgHeldFirearm->IsReloading()) {
						fgWeaponString = "重 新 装 填"; //Reloading
						...
						fgWeaponString = fgHeldFirearm->GetRoundInMagCount() < 0 ? "无 限 弹 药 !" : std::to_string(fgHeldFirearm->GetRoundInMagCount()); //Infinite
					}
				}
				...
				if (bgHeldFirearm) {
					if (bgHeldFirearm->IsReloading()) {
						bgWeaponString = "重 新 装 填"; //Reloading
						int barColorIndex = 77;
				...
				pSymbolFont->DrawAligned(&allegroBitmap, drawPos.GetFloorIntX() - pSymbolFont->CalculateWidth(str, 1) - 3, drawPos.GetFloorIntY() + m_HUDStack, str, GUIFont::Left, 0, 0, 0, 1);
			...
			if (m_Controller.IsState(PIE_MENU_ACTIVE) || !m_EquipHUDTimer.IsPastRealMS(700)) {
				...
				//这里是环形菜单中的，显示手持武器名字的，也能改成中文
				std::string equippedItemsString = (fgEquippedItem ? CN_getNameFromDescription(fgEquippedItem->GetDescription(), fgEquippedItem->GetPresetName()) : "空") + (bgEquippedItem ? " | " + CN_getNameFromDescription(bgEquippedItem->GetDescription(), bgEquippedItem->GetPresetName()) : "");
			...
			std::snprintf(str, sizeof(str), "没 有 胳 膊 !"); //NO ARM!
		...
		if (m_pJetpack && m_Status != INACTIVE && !m_Controller.IsState(PIE_MENU_ACTIVE) && (m_Controller.IsState(BODY_JUMP) || !m_pJetpack->IsFullyFueled())) {
			...
			str[1] = 0;
			pSymbolFont->DrawAligned(&allegroBitmap, drawPos.GetFloorIntX() - 7, drawPos.GetFloorIntY() + m_HUDStack, str, GUIFont::Centre, 0, 0, 0, 1);
		...
		if (!m_Controller.IsState(PIE_MENU_ACTIVE) && m_pItemInReach) {
			//这个地方呢，是你操控的小人头顶的捡起物品提示，这个地方也可以改成中文的
			std::snprintf(str, sizeof(str), " %c %s", -49, CN_getNameFromDescription(m_pItemInReach->GetDescription(), m_pItemInReach->GetPresetName()).c_str());
		...
		switch (pieSlice->GetType()) {
			...
			case PieSliceType::Reload:
				...
				if (pieSlice->GetType() == PieSliceType::Pickup) {
					if (m_pFGArm || (m_pBGArm && m_pItemInReach->IsOneHanded())) {
						...
						//环形菜单捡东西按钮的显示，汉化一下
						pieSlice->SetDescription("捡起 " + CN_getNameFromDescription(m_pItemInReach->GetDescription(), m_pItemInReach->GetPresetName())); // Pick Up
						//pieSlice->SetDescription("Pick Up " + m_pItemInReach->GetPresetName());
					} else {
						pieSlice->SetEnabled(false);
						pieSlice->SetDescription("没有胳膊"); //No Arm
					}
				} else {
					...
					if (fgHeldDevice || bgHeldDevice) {
						...
						pieSlice->SetEnabled(false);
						pieSlice->SetDescription(m_pFGArm ? "手上空空的" : "没有胳膊"); //"Not Holding Anything" : "No Arm"
					}
				}
				break;
			case PieSliceType::NextItem:
				if (!IsInventoryEmpty() && m_pFGArm) {
					pieSlice->SetEnabled(m_Status != INACTIVE);
					pieSlice->SetDescription("下一件物品"); //Next Item
				} else {
					pieSlice->SetEnabled(false);
					pieSlice->SetDescription(m_pFGArm ? "未持有任何物品" : "没有胳膊"); //"Not Holding Anything" : "No Arm"
				}
				break;
			case PieSliceType::PreviousItem:
				if (!IsInventoryEmpty() && m_pFGArm) {
					pieSlice->SetEnabled(m_Status != INACTIVE);
					pieSlice->SetDescription("上一件物品"); //Prev Item
				} else {
					pieSlice->SetEnabled(false);
					pieSlice->SetDescription(m_pFGArm ? "未持有任何物品" : "没有胳膊"); //"Not Holding Anything" : "No Arm"
				}
			...
			case PieSliceType::Drop:
				if (const MovableObject* equippedFGItem = GetEquippedItem()) {
					pieSlice->SetEnabled(m_Status != INACTIVE);
					//环形菜单扔下主手物品的按钮，汉化一下
					pieSlice->SetDescription("扔下 " + CN_getNameFromDescription(equippedFGItem->GetDescription() , equippedFGItem->GetPresetName()));
					//pieSlice->SetDescription("Drop " + equippedFGItem->GetPresetName());
				} else if (const MovableObject* equippedBGItem = GetEquippedBGItem()) {
					// 环形菜单扔下副手物品的按钮，汉化一下
					pieSlice->SetDescription("扔下 " + CN_getNameFromDescription(equippedBGItem->GetDescription(), equippedBGItem->GetPresetName()));
					//pieSlice->SetDescription("Drop " + equippedBGItem->GetPresetName());
					pieSlice->SetEnabled(m_Status != INACTIVE); 
				} else if (!IsInventoryEmpty() && !m_pFGArm) {
					pieSlice->SetEnabled(m_Status != INACTIVE);
					pieSlice->SetDescription("扔下库存"); //Drop Inventory
				} else {
					pieSlice->SetEnabled(false);
					pieSlice->SetDescription((m_pFGArm || m_pBGArm) ? "未持有任何物品" : "没有胳膊"); //"Not Holding Anything" : "No Arm"
				}
				break;
		}
	}
