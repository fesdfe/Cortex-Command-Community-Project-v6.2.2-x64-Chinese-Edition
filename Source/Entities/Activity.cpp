...
std::string Activity::GetTeamName(int whichTeam) const {
	if (whichTeam >= Teams::TeamOne && whichTeam < Teams::MaxTeamCount) {
		return m_TeamActive[whichTeam] ? m_TeamNames[whichTeam] : "未启用队伍"; //Inactive Team
	}
	return "";
}
...
void Activity::HandleCraftEnteringOrbit(ACraft* orbitedCraft) {
	...
	std::string craftText = "返回的投送飞船"; //Returned craft
	if (!orbitedCraft->IsInventoryEmpty()) {
		craftText += "及其中的货物"; // + cargo
	}
	if (totalValue > 0.0F) {
	...
	for (int player = Players::PlayerOne; player < Players::MaxPlayerCount; ++player) {
		if (m_IsActive[player]) {
			if (brainOnBoard && orbitedCraft == GetPlayerBrain(static_cast<Players>(player))) {
				...
				g_FrameMan.SetScreenText("你的主脑被驱逐出了星球 !", ScreenOfPlayer(static_cast<Players>(player)), 0, 3500); //YOUR BRAIN HAS BEEN EVACUATED BACK INTO ORBIT!
